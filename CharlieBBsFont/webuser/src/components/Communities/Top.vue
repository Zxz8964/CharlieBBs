  ...
  <v-card :class='!alt && "padding-bot"'>
    ...
    <v-list>
      <router-link v-for='(community, i) in prefixedCommunities' :key='community.id' :to='community.name'>
        <v-list-item>
          <span>{{ i + 1 }}</span>
          <Avatar :communityName='community.name.substring(2)' /> <v-card-text class='community-name'>{{ community.name
            }}</v-card-text>
        </v-list-item>
      </router-link>
    </v-list>
...
export default {
  ...
  computed: {
    ...
    prefixedCommunities() {
      ...
      return this.communities.filter(e => !this.category || (this.category === e.category))
        .map(e => ({ ...e, name: `r/${e.name}` }))
        .slice(0, !this.alt ? 5 : 100);
    }
  },
  methods: {
    onClick() {
